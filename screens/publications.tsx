...
export default function PublicationsScreen({
  ...
  const cardWidth = useMemo(() => (windowWidth - pagePadding*2*themeConstant - cardSpace*themeConstant)/2, []);

  // calculate left card list
  const leftCardList = publications
      .map((p, i) => i)
      .filter((p, j) => {
          if (expandedIndex < 0) { // 没有展开卡
            return checkOdd(j)
          } else if (checkOdd(expandedIndex)) { // 展开的卡片是偶数
