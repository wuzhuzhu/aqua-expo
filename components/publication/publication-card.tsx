...
const PublicationCard = function ({
	p,
	cardWidth,
	cardSpace,
	isLeft,
	...
	setExpandedIndex,
	navigation,
}: IPublicationCardType) {
	// show up Animation
	const [animate, showUpAnimationStyles] = useShowUpAnimation();
	useEffect(animate, []);

	// animations
	const isChaptersShow = useMemo(
		() => expandedIndex === rank,
		[rank, expandedIndex]
	);
	const wrapperWidth = useMemo(
		() => (expandedIndex === rank ? cardWidth * 2 + cardSpace : cardWidth),
		[rank, expandedIndex]
	);
	const chapters = Array.isArray(p?.chapters) ? p.chapters : [];
	const imgHeight = (cardWidth * 4) / 3;
	const mr = isChaptersShow ? 0 : isLeft ? cardSpace : 0;
	// btn show up alert
	...
	const navigatePublication = function (
		navigation,
		pdfUrl,
		page = 1 as number | undefined,
		title
	) {
		let path = `${PDF_URL_BASE}/${encodeURIComponent(pdfUrl)}`;
		if (page) path += `/${page}`;
	...
	const animateBg = useAnimatedStyle(() => {
		const backgroundColor = interpolateColor(scrollDistance.value, inputRange, [
			"#1469a8",
			"#37b055",
		]);
		const margin = interpolate(scrollDistance.value, inputRange, [0, 5]);
		return {
			backgroundColor,
			margin,
		};
	...
	return (
		...
		>
			...
			>
					...
					<AspectRatio ratio={3 / 4} height={imgHeight}>
						...
						>
							...
							<NBAnimatedView
								...
								top="3"
								py="1.5"
								style={rank === 0 ? showUpAnimationStyles : null}
							>
								<Box style={[{ overflow: "hidden" }]}>
									<BetterButton onPressBtn={toggleChapter}>
										<AnimatedLinearGradient
											style={[
												animatedStyles,
												{ width: "600%", alignItems: "flex-start" },
											]}
											height={18}
										...
										>
											<Icon
												mr="1"
												as={Feather}
												name={isChaptersShow ? "book-open" : "book"}
												color="warmGray.50"
												size="sm"
											/>
											<NBAnimatedText
												...
												color="warmGray.50"
												fontSize="xs"
											>{`${chapters.length} Chapters`}</NBAnimatedText>
										</Center>
									</BetterButton>
								</Box>
							</NBAnimatedView>
				...
				{isChaptersShow ? (
					<Box h={imgHeight} bg="muted.100" shadow={3}>
						<Animated.ScrollView
						...
						>
							<Column
								bg="muted.100"
								style={{ flexWrap: "wrap" }}
								pr={wrapperWidth - cardWidth}
							>
								{chapters.map(c => (
									<ChapterCard
										key={`chapter-${c.id}`}
										c={c}
										rowWidth={wrapperWidth - cardWidth}
										pdfUrl={p.pdfUrl}
